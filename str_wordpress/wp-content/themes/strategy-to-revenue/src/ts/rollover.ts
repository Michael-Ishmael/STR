...
abstract class  RolloverTweenBase {
    ...
    protected abstract overImpl(tweenContainer:StrElementTween, duration: number, ease: Ease): void;
...
export class CssPropertyTween extends RolloverTweenBase implements IRolloverTween {
    ...
    protected overImpl(tweenContainer:StrElementTween, duration: number, ease: Ease): void {
        let cssTransformObj: {[key:string]: any} = {};
...
export class MovePercentageOfParent extends RolloverTweenBase implements IRolloverTween{
    ...
    protected overImpl(tweenContainer:StrElementTween, duration: number, ease: Ease): void {
        ...
        let parentDimension = this.direction.toLowerCase() === 'y' ?
            tweenContainer.jElement.parent().height() : tweenContainer.jElement.parent().width();
        let to = parentDimension * this.percentage;
        to = to - (parentDimension * this.percentage);
        let vars:{ [key: string]: any } =  { "ease": ease};
